    ...
    def process_frame(self, frame):
        ...
        for veh_id, bbox in tracked_bboxes.items():
            ...
            if veh_id not in self.vehicle_records:
                ...
                current_watchlist = database.get_watchlist_plates()
                is_flagged = plate_text in current_watchlist
                
                # Log ANPR read
                if plate_text and plate_text != "UNKNOWN":
                    database.insert_anpr_read(plate_text, self.camera_id, float(conf * 100))
                ...
                if is_flagged:
                    database.insert_alert('BLACKLISTED_VEHICLE', 'critical', plate_text, self.camera_id)
                    alerts.append({
