...
export default function InteractiveMap({
  ...
  useEffect(() => {
    ...
    // Draw immediate fallback line so route is always visible
    ...
    // Always add START/END badges immediately — do NOT wait for OSRM
    trajectoryPoints.forEach((pt, idx) => {
      ...
      `
      const badgeIcon = L.divIcon({ html, className: 'trajectory-badge-pin', iconSize: [80, 24], iconAnchor: [40, 12] })
      L.marker([pt.lat, pt.lng], { icon: badgeIcon }).addTo(group)
    })

    // Fit map bounds with generous padding so the full route is always visible
    try {
    ...
    if (rawCoords.length >= 2) {
      // Attempt OSRM road routing to draw route along real roads
      ...
      fetch(url)
        ...
        .then((data) => {
          if (data.routes && data.routes[0] && data.routes[0].geometry) {
            ...
            // Remove fallback dashed line, replace with real road line
            group.removeLayer(fallbackPolyline)

            // Outer glow casing
            L.polyline(roadCoords, {
              color: '#0284c7', weight: 8, opacity: 0.45, lineCap: 'round', lineJoin: 'round',
            }).addTo(group)

            // Main vibrant road line
            L.polyline(roadCoords, {
              color: '#38bdf8', weight: 4, opacity: 0.95, lineCap: 'round', lineJoin: 'round',
            }).addTo(group)

            map.fitBounds(L.latLngBounds(roadCoords), { padding: [80, 80], maxZoom: 14 })
          }
        })
        .catch((err) => {
