...
def _seed_initial_data(c, conn):
    ...
    if not c.execute("SELECT COUNT(*) FROM vehicles").fetchone()[0]:
        ...
        vehs = [
            ...
            ("V004", "UTX-VH-00127", "Car", "ΓÇö", 78, "Lost", "CAM-006", 0, "10:32:41"),
    ...
    if not c.execute("SELECT COUNT(*) FROM settings").fetchone()[0]:
        defaults = [
            ...
            ("site_name", "UrbanTrax AI ΓÇö Bengaluru"),
        ]
        now = datetime.utcnow().isoformat()
        for k, v in defaults:
...
# ΓöÇΓöÇΓöÇ ML Pipeline (optional ΓÇö graceful fallback if model files not present) ΓöÇΓöÇΓöÇ
...
try:
    from backend.app.routers.anpr import process_frame as process_anpr_frame, ANPRProcessRequest
    @app.post("/api/anpr/process_frame")
    def api_process_anpr_frame(req: ANPRProcessRequest):
        return process_anpr_frame(req)
except Exception:
    pass
...
def generate_synthetic_frame(camera_id: str, frame_num: int):
    ...
    # Camera label
    cv2.putText(img, f"{camera_id} ΓÇö LIVE", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
...
<body><h1>UrbanTrax AI ΓÇö Backend API v2.0</h1>
